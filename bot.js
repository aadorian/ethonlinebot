...
const ABOUT_0x = "0x is important infrastructure for the emerging crypto economy and enables markets to be created that couldn't have existed before. As more assets become tokenized, public blockchains provide the opportunity to establish a new financial stack that is more efficient, transparent, and equitable than any system in the past.";
const ABOUT_PARASWAP = "ParaSwap API allows any developer to retrieve Token prices as well as making swaps between EVM based assets (Ethereum mainnet, Polygon, and others";
...
client.on("message", async (msg) => {
    try {
        if (msg.content === "!0x") {
            ...
            const signature = await order.getSignatureWithKey(process.env.PRIVATE_KEY);
            //Order and signature example in Ox
            console.log(order);
            console.log(signature);
            const contractEmbed = new discord_js_1.MessageEmbed()
                .setURL("URL_DISCORD")
                ...
                inline: false,
            }, { name: "chain ID", value: `${order.chainId}`, inline: false }, { name: "about 0x", value: ABOUT_0x, inline: true })
                .setTimestamp()
                .setImage(IMAGE_0x)
                .setTimestamp()
                .setFooter(BOT_NAME);
        ...
        if (msg.content === "!paraswap") {
            ...
            const priceRoute = await paraSwap.getRate(TOKEN_SOURCE, TOKEN_DESTINATION, TOKEN_AMOUNT);
            console.log(priceRoute);
            const paraSwapEmbed = new discord_js_1.MessageEmbed()
                .setURL("URL_DISCORD")
                ...
                .setURL(URL_PARASWAP)
                .addField(`msg`, "" + `ETH / DAI using paraswap: `, false)
                .addField(`blockNumber`, "" + `${priceRoute.blockNumber}`, true)
                .addField(`network`, "" + `${priceRoute.network}`, true)
                .addField(`source token`, "" + `${priceRoute.srcToken}`, true)
                .addField(`source amount`, "" + `${priceRoute.srcAmount}`, true)
                .addField(`destination token`, "" + `${priceRoute.destToken}`, true)
                .addField(`destination amount`, "" + `${priceRoute.destAmount}`, true)
                .addField(`gasCost USD`, "" + `${priceRoute.gasCostUSDUSD}`, true)
                .addField(`gasCost`, "" + `${priceRoute.gasCost}`, true)
                .addField(`method`, "" + `${priceRoute.contractMethod}`, true)
                .setTimestamp()
        ...
        if (msg.content === "!pocket") {
            ...
            const pocketEmbed = new discord_js_1.MessageEmbed()
                .setURL("URL_DISCORD")
                ...
                .setTitle("Pocket API")
                .setURL(URL_POCKET)
                .addField(`configuration`, "maxDispatchers: " + `${pocketInstance.configuration.maxDispatchers}`, false)
